...
import { deduplicateEvents, stripHtml, cleanTitle } from "@/lib/data/cleanPurdueData";
import { fetchPurdueApi } from "@/lib/data/fetchPurdue";
import type { TypesenseOrganizationDocument } from "@/lib/typesense/schema";
import type { TypesenseEventDocument } from "@/lib/typesense/schema";
import type { RawPurdueEventWrapper } from "@/lib/data/normalizePurdueEvent";
...
async function runPipelineTests() {
  ...
  const deptsData = await fetchPurdueApi<{ departments?: unknown[] }>(
    "https://events.purdue.edu/api/2/departments?pp=100"
  );
  const groupsData = await fetchPurdueApi<{ groups?: unknown[] }>(
    "https://events.purdue.edu/api/2/groups?pp=100"
  );

  const orgDocs: TypesenseOrganizationDocument[] = [];
  ...
  const eventsData = await fetchPurdueApi<{ events?: unknown[] }>(
    "https://events.purdue.edu/api/2/events?pp=100&days=60"
  );
  const rawEvents = eventsData?.events || [];

  let rawHadGeo = 0;
  let resolvedGeo = 0;
  const normalizedEvents: TypesenseEventDocument[] = [];

  for (const r of rawEvents) {
    const wrapper = r as RawPurdueEventWrapper;
    const e = wrapper.event || wrapper;
    if (e.geo?.latitude && e.geo?.longitude) rawHadGeo++;
    const doc = normalizePurdueEvent(wrapper);
    if (doc) {
